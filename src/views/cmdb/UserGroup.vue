  ...
  <div>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="ref" :model="tempData" :rules="rules" class="el-form">
        <el-form-item label="姓名" label-width="80px" prop="title">
          <el-input v-model="tempData.title" />
        </el-form-item>
        <el-form-item label="备注" label-width="80px">
          <el-input v-model="tempData.remark" type="textarea" />
        </el-form-item>
        <el-form-item label="组成员" label-width="80px">
          <el-transfer
            v-model="tempData.users"
            :titles="['未加入人员', '已加入人员']"
            :data="userList"
            filter-method
            label="userList.name"
...
import { UserGroup as masterApi } from '@/api/cmdb'
import { User } from '@/api/cmdb'
...
export default {
  name: 'Idc',
  data() {
    const validateisUserlist = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('组成员不能为空'))
      }
    }
    return {
      userList: [],
      ...
      rules: {
        title: [{ required: true, message: '姓名必填', trigger: 'blur' }],
        users: [{ required: true, trigger: 'blur', validator: validateisUserlist }]
      }
    }
  },
  created() {
